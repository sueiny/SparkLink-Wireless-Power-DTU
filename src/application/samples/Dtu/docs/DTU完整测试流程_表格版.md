...
|----------|-------------------------------|---------|-------------------------------------------------------------------|------------------------------------------------|
| 读取     | READ_DEV_INFO                 | 0x01    | AA 55 01 01 00 00 50 18                                           | 响应 0x81；status=00；返回 role/mac/name。     |
| 读取     | READ_UART_CFG                 | 0x02    | AA 55 02 02 00 00 A0 5C                                           | 响应 0x82；status=00；默认 07 00 01 08。       |
| 读取     | READ_MODBUS_CFG               | 0x03    | AA 55 03 03 00 00 F0 60                                           | 响应 0x83；status=00；默认 item_count=8。      |
...
| 读取     | READ_WL_NODE_CFG              | 0x07    | AA 55 07 07 06 00 A1 A2 A3 A4 A5 A6 80 F3                         | ROOT 下返回该白名单 node 的 uart/modbus 子配置。 |
...
| ROOT     | ADD_WL_ITEM                   | 0x14    | AA 55 14 0A 0E 00 A1 A2 A3 A4 A5 A6 07 44 54 55 5F 4E 30 31 07 7B | ROOT 下添加 DTU_N01。                          |
...
| ROOT     | SET_WL_NODE_CFG               | 0x17    | AA 55 17 08 0F 00 A1 A2 A3 A4 A5 A6 07 00 01 08 02 01 05 02 05 91 F7 | ROOT 下设置该白名单 node 的 uart/modbus 子配置。 |
...
| 2        | READ_DEV_INFO     | AA 55 01 01 00 00 50 18 | 0x81；status=00；返回 role/mac/name  |                  |              | 设备名可能是 Kconfig 配置名或 DTU_xxxxxx。 |
| 3        | READ_UART_CFG     | AA 55 02 02 00 00 A0 5C | 0x82；status=00；默认 07 00 01 08    |                  |              | 即 115200,None,1,8。                       |
| 4        | READ_MODBUS_CFG   | AA 55 03 03 00 00 F0 60 | 0x83；status=00；默认 item_count=8   |                  |              | 默认 addr=0..7，type=0x05。               |
...
| 4        | ADD_WL_ITEM       | 当前为 ROOT          | AA 55 14 0A 0E 00 A1 A2 A3 A4 A5 A6 07 44 54 55 5F 4E 30 31 07 7B | status=00；添加 DTU_N01                       |                  |              |
| 5        | READ_ROOT_WL_ALL  | 当前为 ROOT          | AA 55 04 04 00 00 40 D5                                           | status=00；能读到 A1:A2:A3:A4:A5:A6 / DTU_N01 |                  |              |
| 6        | READ_WL_NODE_CFG  | 当前为 ROOT 且已添加 | AA 55 07 07 06 00 A1 A2 A3 A4 A5 A6 80 F3                         | status=00；返回该 node 默认继承的 uart/modbus |                  |              |
| 7        | SET_WL_NODE_CFG   | 当前为 ROOT 且已添加 | AA 55 17 08 0F 00 A1 A2 A3 A4 A5 A6 07 00 01 08 02 01 05 02 05 91 F7 | status=00；写入 node 子配置                 |                  |              |
| 8        | DEL_WL_ITEM       | 当前为 ROOT 且已添加 | AA 55 15 0B 06 00 A1 A2 A3 A4 A5 A6 EA BD                         | status=00；再次读取应不存在                   |                  |              |
| 9        | ADD_WL_ITEM       | 当前为 ROOT          | AA 55 14 0A 0E 00 A1 A2 A3 A4 A5 A6 07 44 54 55 5F 4E 30 31 07 7B | status=00；重新添加                           |                  |              |
| 10       | CLEAR_WL          | 当前为 ROOT          | AA 55 16 0C 00 00 C4 6F                                           | status=00；白名单清空                         |                  |              |
| 11       | READ_ROOT_WL_ALL  | 当前为 ROOT          | AA 55 04 04 00 00 40 D5                                           | status=00；wl_total=0 或 item_count=0         |                  |              |
...
| 5        | NODE 新增白名单     | 当前为 NODE  | AA 55 14 0A 0E 00 A1 A2 A3 A4 A5 A6 07 44 54 55 5F 4E 30 31 07 7B | status=06 ROLE_MISMATCH |                  |              |
...
| 5        | 新增白名单           | AA 55 14 0A 0E 00 A1 A2 A3 A4 A5 A6 07 44 54 55 5F 4E 30 31 07 7B | status=00                        |              |              | 添加 DTU_N01。         |
...
| 11       | 重启后读白名单       | AA 55 04 04 00 00 40 D5                                           | 能读到 DTU_N01                   |              |              | 确认白名单保留。       |
...
| 12       | ADD_WL_ITEM       | AA 55 14 0A 0E 00 A1 A2 A3 A4 A5 A6 07 44 54 55 5F 4E 30 31 07 7B | status=05 NOT_CONFIG        |                  |              | 运行模式拒绝。           |
| 13       | READ_WL_NODE_CFG  | AA 55 07 07 06 00 A1 A2 A3 A4 A5 A6 80 F3                         | status=05 NOT_CONFIG        |                  |              | 运行模式拒绝。           |
| 14       | DEL_WL_ITEM       | AA 55 15 0B 06 00 A1 A2 A3 A4 A5 A6 EA BD                         | status=05 NOT_CONFIG        |                  |              | 运行模式拒绝。           |
| 15       | CLEAR_WL          | AA 55 16 0C 00 00 C4 6F                                           | status=05 NOT_CONFIG        |                  |              | 运行模式拒绝。           |
| 16       | SET_WL_NODE_CFG   | AA 55 17 08 0F 00 A1 A2 A3 A4 A5 A6 07 00 01 08 02 01 05 02 05 91 F7 | status=05 NOT_CONFIG     |                  |              | 运行模式拒绝。           |
| 17       | COMMIT            | AA 55 20 0D 00 00 9A 27                                           | status=05 NOT_CONFIG        |                  |              | 运行模式拒绝。           |
| 18       | FACTORY_RESET     | AA 55 22 0F 00 00 3A 5F                                           | status=05 NOT_CONFIG        |                  |              | 运行模式拒绝。           |
| 19       | REBOOT            | AA 55 21 0E 00 00 6B DB                                           | status=00；设备重启         |                  |              | 运行模式允许。           |
...
| 5        | READ_MODBUS_CFG         | AA 55 03 03 00 00 F0 60                                | modbus_count=8                   |              |              |
...
| **序号** | **验收项**                 | **通过标准**                                                                                                    | **是否通过** | **备注** |
|----------|----------------------------|-----------------------------------------------------------------------------------------------------------------|--------------|----------|
| 1        | 配置模式全部读取命令通过   | READ_DEV_INFO / READ_UART_CFG / READ_MODBUS_CFG / READ_ROOT_WL_ALL / READ_ROOT_POWER / GET_MODE_STATUS / READ_WL_NODE_CFG 符合预期 |              |          |
| 2        | 配置模式全部写入命令通过   | SET_ROLE / SET_UART_CFG / SET_MODBUS_CFG / SET_ROOT_POWER / ADD/DEL/CLEAR_WL / SET_WL_NODE_CFG 均符合预期      |              |          |
...
| 6        | FACTORY_RESET 正确         | 配置恢复默认：NODE、115200 8N1、modbus_count=8、power=5、wl_count=0                                             |              |          |
...
|-----------------|-------------------------------------------------------------------|------------------|-------------------------------|--------------|--------------|----------|
| READ_DEV_INFO   | AA 55 01 01 00 00 50 18                                           |                  | 返回 role/mac/name，status=00 |              |              |          |
...
| ADD_WL_ITEM     | AA 55 14 0A 0E 00 A1 A2 A3 A4 A5 A6 07 44 54 55 5F 4E 30 31 07 7B |                  | status=00                     |              |              |          |
