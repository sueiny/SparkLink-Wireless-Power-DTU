 ...
 * Description: mbedtls aes harden implement short term.
 *
 * Create: 2023-12-23
*/
...
static int inner_set_key(td_handle keyslot_handle, const unsigned char *key, unsigned int key_len)
{
    int ret;
    ...
    const unsigned char *src, unsigned char *dst, unsigned int data_len, unsigned int is_decrypt)
{
    ...
    crypto_chk_goto(ret != CRYPTO_SUCCESS, exit_unlock, "drv_cipher_symc_create failed\n");

    ret = drv_keyslot_create(&keyslot_handle, CRYPTO_KEYSLOT_TYPE_MCIPHER);
    crypto_chk_goto(ret != CRYPTO_SUCCESS, exit_symc_destroy, "drv_keyslot_create failed\n");

    ret = drv_cipher_symc_attach(symc_handle, keyslot_handle);
    crypto_chk_goto(ret != CRYPTO_SUCCESS, exit_keyslot_destroy, "drv_cipher_symc_attach failed\n");

    ret = inner_set_key(keyslot_handle, key, key_len);
    crypto_chk_goto(ret != CRYPTO_SUCCESS, exit_keyslot_destroy, "drv_cipher_symc_set_key failed\n");

    ret = drv_cipher_symc_set_config(symc_handle, symc_ctrl);
    crypto_chk_goto(ret != CRYPTO_SUCCESS, exit_keyslot_destroy, "drv_cipher_symc_set_config failed\n");

    src_buf.phys_addr = (uint32_t)(uintptr_t)src;
    dst_buf.phys_addr = (uint32_t)(uintptr_t)dst;
    if (is_decrypt == 0) {
        ret = drv_cipher_symc_encrypt(symc_handle, &src_buf, &dst_buf, data_len);
    } else {
        ret = drv_cipher_symc_decrypt(symc_handle, &src_buf, &dst_buf, data_len);
    }
    crypto_chk_goto(ret != CRYPTO_SUCCESS, exit_keyslot_destroy, "drv_cipher_symc_crypto failed\n");

    ret = drv_cipher_symc_get_config(symc_handle, symc_ctrl);
    crypto_chk_goto(ret != CRYPTO_SUCCESS, exit_keyslot_destroy, "drv_cipher_symc_get_config failed\n");

exit_keyslot_destroy:
    (void)drv_keyslot_destroy(keyslot_handle);
exit_symc_destroy:
    (void)drv_cipher_symc_destroy(symc_handle);
exit_unlock:
